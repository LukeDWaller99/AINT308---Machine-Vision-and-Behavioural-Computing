...
\textbf{OpenVC} includes a built in method for retrieving the values of a pixel in an image.The code iterates through two loops, one to look at all the rows and the second to go through all the colum if the R(red), G(green), or B(blue) value if more than $1.5$ times larger than the other R, G, B values, then the pixel is deemed to be that colour. 
...
A combination of the three previous example of cross correlation could provide a lightweight initial set of tests backed up by a more computationally intensive but more robust solution. This could be used to evaluate 
